...
$logacc=true;
	...
	if($auditObj)
	{
		...
		{
			$logacc=false;
			$message=mysprintf("",array($auditObj->LoginAccess()));
		}
	}
...
//   	 username and password are stored in the database
	...
	if(NeedQuotes($cPasswordFieldType))
		...
		$strSQL = "select * from `agenda_petugas` where ".AddFieldWrappers($cUserNameField).
	...
	if($logged && $isCaptchaOk)
	{
		...
		$_SESSION["GroupID"] = $data["Username"];
					...
					$_SESSION["OwnerID"] = $data["id_user"];
	$_SESSION["_agenda_hadir_OwnerID"] = $data["id_user"];
			$_SESSION["_agenda_petugas_OwnerID"] = $data["id_user"];
		if($auditObj)
		{
			$auditObj->LogLogin();
...
if(@$_GET["message"]=="expired")
	$message = "Sesi anda telah habis masa berlakunya. Silahkan login kembali.";
