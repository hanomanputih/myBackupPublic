...
if($mastertable=="agenda_hadir")
{
	$where ="";
		$where.= GetFullFieldName("id_hadir")."=".make_db_value("id_hadir",$_SESSION[$strTableName."_masterkey1"]);
}
...
if ( $rowcount ) {
	...
	//	id_hadir - 
		    $value="";
				$value=DisplayLookupWizard("id_hadir",$data["id_hadir"],$data,$keylink,MODE_PRINT);
			$row["id_hadir_value"]=$value;
	//	Hasil - HTML
		    $value="";
				$value = GetData($data,"Hasil", "HTML");
			$row["Hasil_value"]=$value;
	//	Catatan - HTML
		    $value="";
				$value = GetData($data,"Catatan", "HTML");
			$row["Catatan_value"]=$value;
	//	Naskah - Document Download
		    $value="";
				$value = GetData($data,"Naskah", "Document Download");
			$row["Naskah_value"]=$value;
	$rowinfo[]=$row;
	}
	$xt->assign_loopsection("details_row",$rowinfo);
} else {
}
$xt->display("agenda_hasil_detailspreview.htm");
if($mode!="inline")
