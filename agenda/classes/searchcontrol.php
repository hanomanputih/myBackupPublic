...
class SearchControl {
	...
	function getCtrlSearchTypeOptions($fName, $selOpt, $not) 
	{
		...
		if ($fType == EDIT_FORMAT_DATE || $fType == EDIT_FORMAT_TIME)
		{
			...
			$options.="<OPTION VALUE=\"More than\" ".(($selOpt=="More than" && !$not)?"selected":"").">"."Lebih dari"."</option>";
			$options.="<OPTION VALUE=\"Less than\" ".(($selOpt=="Less than" && !$not)?"selected":"").">"."Kurang dari"."</option>";
			$options.="<OPTION VALUE=\"Between\" ".(($selOpt=="Between" && !$not)?"selected":"").">"."Antara"."</option>";
			$options.="<OPTION VALUE=\"Empty\" ".(($selOpt=="Empty" && !$not)?"selected":"").">"."Kosong"."</option>";
		}
	...
	{		
		$html = '<img style="visibility: hidden;" id = "'.$this->getDelButtonId($fName, $recId).'" class="searchPanelButton" src="images/search/closeRed.gif" alt="'."".'"  onclick="searchController'.$this->id.'.delCtrl(\''.htmlspecialchars(jsreplace($fName)).'\', '.$recId.');">';
		return $html;
	}
