...
class SearchPanelSimple extends SearchPanel {
	...
	function buildSearchPanel($xtVarName) 
	{
		...
		{
			$mrgStyle = 'style="padding-';
			if($this->pageObj->xt->getReadingOrder()=='RTL')
				$mrgStyle .= 'right';
			else
				$mrgStyle .= 'left';
			$mrgStyle .= ':203px;"';
			$this->pageObj->xt->assign("marginStyle", $mrgStyle);
		}
	}
	...
	function searchAssign() {
		...
		$searchOpt_mess = ($searchPanelAttrs['srchOptShowStatus'] ? "Hilangkan Opsi pencarian" : "Tampakkan opsi pencarian");
