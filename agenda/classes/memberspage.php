...
{	
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `agenda_ugmembers` where UserName='".db_addslashes($user)."'";
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `agenda_ugmembers` (UserName,GroupID) values ('".htmlspecialchars($user)."',".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `agenda_ugmembers` (UserName,GroupID) select '".htmlspecialchars($user)."',GroupID from `agenda_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	{
		//	select members list		
		$trs = db_query("select UserName,GroupID from `agenda_ugmembers` order by UserName,GroupID",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select GroupID,Label from `agenda_uggroups` order by Label",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function addCommonJs() 
	{
		...
		$this->AddJSCode("
			...
			window.SUGGEST_TABLE='searchsuggest.php?table=".GetTableURL($this->tName)."';
