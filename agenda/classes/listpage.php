...
class ListPage extends RunnerPage 
{
	...
	function addCommonHtml() 
	{
		...
		if($this->is508) {
			$this->body["begin"].= "<a href=\"#skipdata\" title=\"".""."\" style=\"width:1px;height:1px;overflow:hidden;display:block;\">".""."</a>";
			$this->body["begin"].= "<a href=\"#skipmenu\" title=\"".""."\" style=\"width:1px;height:1px;overflow:hidden;display:block;\">".""."</a>";
			$this->body["begin"].= "<a href=\"#skipsearch\" title=\"".""."\" style=\"width:1px;height:1px;overflow:hidden;display:block;\">".""."</a>";
			$this->body["begin"].= "<a href=\"templates/helpshortcut.htm\" title=\"".""."\" style=\"width:1px;height:1px;overflow:hidden;display:block;\">".""."</a>";
		}
	...
	function prepareForResizeColumns()
	{
		if($this->mode!=LIST_AJAX)
		{
			...
			{
				$this->AddJSFile("yui/yahoo");
				$this->AddJSFile("yui/cookie", "yui/yahoo");		
				$this->AddJSFile("yui/dom", "yui/cookie");		
				$this->AddJSFile("yui/event", "yui/dom");
				$this->AddJSFile("yui/element", "yui/event");
				$this->AddJSFile("yui/datasource", "yui/element");		
				$this->AddJSFile("yui/dragdrop", "yui/datasource");
				$this->AddJSFile("yui/datatable", "yui/dragdrop");
				$this->AddJSFile("yui/json", "yui/datatable");
				$this->AddJSFile("yui/resize", "yui/json");		
			}
			else 
			{
				$this->AddJSFile("yui/yuiAll");
			}
			$this->AddJSFile("resizebleGrid");
	...
	function deleteRecords() 
	{
		...
		foreach($this->selectedRecs as $keys) 
		{
			...
			$retval = true;
			if(function_exists("AfterDelete") || function_exists("BeforeDelete") || $this->audit) 
			{
			...
			if(function_exists("BeforeDelete"))
				$retval = BeforeDelete($where, $deleted_values, $message);
				...
				if(@$_REQUEST["a"] == "delete")
				{
					if($retval) 
					{
						...
						if(function_exists("AfterDelete"))
							AfterDelete($where, $deleted_values,$message);
					}
					else
					{
						$this->xt->assignbyref("user_message", $message);
					}
				}
			}
	...
	function deleteSelectedAttrs()
	{
		$this->xt->assign("deleteselectedlink_attrs", "name=\"delete_selected".$this->id."\" onclick=\"
				if(\$('input[@type=checkbox][@checked][@name^=selection]').length && confirm('"."Apa anda ingin menghapus record-record ini?"."')){ frmAdmin".$this->id.".submit();". 
		...
		if(! $this->numRowsFromSQLFromSQL) 
		{
			...
			$this->xt->assign("message",($this->is508 == true ? "<a name=\"skipdata\"></a>" : "").$message);
				...
				if($this->maxPages > 1) 
				{
					...
					if($counterstart != 1) 
					{
						$pagination.= "<a href='JavaScript:GotoPage".$this->id."(1);' style='TEXT-DECORATION: none;'>"."Pertama"."</a>";
						$pagination.= "&nbsp;:&nbsp;";
						$pagination.= "<a href='JavaScript:GotoPage".$this->id."(".($counterstart - 1).");' style='TEXT-DECORATION: none;'>"."Sebelumnya"."</a>";
						$pagination.= "&nbsp;";
					}
					$pagination.= "<b>[</b>";
					...
					if($counterend != $this->maxPages) 
					{
						$pagination.= "&nbsp;<a href='JavaScript:GotoPage".$this->id."(".($counterend + 1).");' style='TEXT-DECORATION: none;'>"."Berikutnya"."</a>";
						$pagination.= "&nbsp;:&nbsp;";
						$pagination.="&nbsp;<a href='JavaScript:GotoPage".$this->id."(".($this->maxPages).");' style='TEXT-DECORATION: none;'>"."Terakhir"."</a>";
					}
	...
	function seekPageInRecSet($strSQL) {
		...
		{			
			if($this->dbType == nDATABASE_MySQL) {			
				...
				if($this->maxPages > 1) {
					$strSQL = AddTopDB2($strSQL, $this->myPage * $this->pageSize);
				}
				$this->recSet = db_query($strSQL, $this->conn);
				db_pageseek($this->recSet, $this->pageSize, $this->myPage);
		...
		if($this->subQueriesSupp && $this->subQueriesSupAccess && ! $this->checkDetailAndMasterFieldTypes()) {
			...
			for($i = 0; $i < count($allDetailsTablesArr); $i ++) {
				if($allDetailsTablesArr[$i]['dispChildCount']|| $allDetailsTablesArr[$i]['hideChild']) {
					...
					$subQ.= " ".$allDetailsTablesArr[$i]['sqlTail'];	
	...
	function fillGridShowInfo(&$rowInfoArr) 
	{
		...
		{		
			...
			for($i = 0; $i < count($allDetailsTablesArr); $i ++) 
			{
				...
				if($allDetailsTablesArr[$i]['previewOnList'] == DP_POPUP) 
				...
				else if($allDetailsTablesArr[$i]['previewOnList'] == DP_INLINE) 
				{
					$record[$allDetailsTablesArr[$i]['dShortTable']."_dtablelink_attrs"] = 
						"id = \"".$allDetailsTablesArr[$i]['dShortTable']."_preview".$this->id."\"
						caption = \"".GetTableCaption(GoodFieldName($allDetailsTablesArr[$i]['dDataSourceTable']))."\" 
						href = \"".$allDetailsTablesArr[$i]['dShortTable']."_list.php?\"";
				}
	...
	{		
		...
		{				
			...
			$record[$fName."_value"]= $span.$record[$fName."_value"]."";
		}
	}
	...
	function proccessDetailGridInfo(&$record, &$data)
	{
		...
		for($i = 0; $i < count($allDetailsTablesArr); $i ++) 
		{
			...
			{
				if($data[$allDetailsTablesArr[$i]['dDataSourceTable']."_cnt"]+ 0)
					$record[$allDetailsTablesArr[$i]['dShortTable']."_childcount"]= true;
				$record[$allDetailsTablesArr[$i]['dShortTable']."_childnumber"] = $data[$allDetailsTablesArr[$i]['dDataSourceTable']."_cnt"];
				$record[$allDetailsTablesArr[$i]['dShortTable']."_childnumber_attr"] = " id='cntDet_".$allDetailsTablesArr[$i]['dShortTable']."_".$this->recId."'";
			}
			
			if($allDetailsTablesArr[$i]['hideChild']) 
			{
				if(!($data[$allDetailsTablesArr[$i]['dDataSourceTable']."_cnt"]+ 0)) 
					$record[$allDetailsTablesArr[$i]['dShortTable']."_dtablelink_attrs"] .= "style='display:none;'";
			}
			
			if($dpreview) 
			{
				if(GetDPType($allDetailsTablesArr[$i]['dDataSourceTable'], $this->tName) == DP_POPUP) 
					$record[$allDetailsTablesArr[$i]['dShortTable']."_dtablelink_attrs"].= " onmouseover=\"RollDetailsLink.showPopup(this,'".$allDetailsTablesArr[$i]['dShortTable']."_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
				
				if(GetDPType($allDetailsTablesArr[$i]['dDataSourceTable'], $this->tName) == DP_INLINE) 
				{					
					$record[$allDetailsTablesArr[$i]['dShortTable']."_dtablelink_attrs"] .= "
						id = \"".$allDetailsTablesArr[$i]['dShortTable']."_preview".$this->recId."\" 
						caption = \"".GetTableCaption(GoodFieldName($allDetailsTablesArr[$i]['dDataSourceTable']))."\"".
						($allDetailsTablesArr[$i]['previewOnList'] != DP_NONE ? 	
						"onclick = \"dpInline".$this->id.".showDPInline('".$allDetailsTablesArr[$i]['dShortTable']."',".$this->recId.",this); return false;\"" : "").
	...
	{
		$gQuery = GetTableData($detailTableInfo['dDataSourceTable'], '.sqlquery', null);
				
		$dObjHaving = $gQuery->Having();
		$dSqlHaving = $dObjHaving->toSql($gQuery);		
		$dSqlGroupBy = $gQuery->GroupByToSql();
				
		$dSqlHead = $detailTableInfo['sqlHead'];
		$dSqlFrom = $detailTableInfo['sqlFrom'];
		$dSqlWhere = $detailTableInfo['sqlWhere'];
		//$sqlTail = $detailTableInfo['sqlTail'];
			
		$detailKeys = GetDetailKeysByMasterTable($this->tName, $detailTableInfo['dDataSourceTable']);
		$masterKeys = GetMasterKeysByDetailTable($detailTableInfo['dDataSourceTable'], $this->tName);		
		...
		$data[$detailTableInfo['dDataSourceTable']."_cnt"]= gSQLRowCount_int($dSqlHead, $dSqlFrom, $dSqlWhere, $dSqlGroupBy, $dSqlHaving, $masterwhere);	
