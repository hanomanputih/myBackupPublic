...
class SQLStatement
{
	...
	function sqlcg()
	{
		...
		return 'select count(*) as '.AddFieldWrappers("c").' from ('.$gsql.') countgroups';
	}
	...
	function getFormattedRow($value)
	{
		...
		{		
			...
			if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_DATABASE_IMAGE)
			{
				...
				{
					$val = "Data Biner Panjang - tidak dapat ditampilkan";
				}
				...
				if(!$this->forExport)
				{
					...
					{
						$val = "<a href=\"getfile.php?table=".$this->shortTName."&filename=".rawurlencode($filename)."&field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['repPage'])).$keylink."\">";
				...
				$val = ProcessLargeText(GetData($value, $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat']), "field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink, "", MODE_LIST);
			}else{
				...
				for($j=0; $j<count($this->fieldsArr); $j++)
				{
					if(is_array($grp['summary']))
					{
						if(is_array($grp['summary'][$this->fieldsArr[$j]['name']]))
						{
							...
							{
								$end["group".$gname."_total".$this->fieldsArr[$j]['goodName']."_max"] =
							...
							if ($this->fieldsArr[$j]['totalMin']){							
								$end["group".$gname."_total".$this->fieldsArr[$j]['goodName']."_min"] =
							...
							if ($this->fieldsArr[$j]['totalAvg']){							
								$end["group".$gname."_total".$this->fieldsArr[$j]['goodName']."_avg"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['AVG'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat']);
							}
							if ($this->fieldsArr[$j]['totalSum']){		
								$end["group".$gname."_total".$this->fieldsArr[$j]['goodName']."_sum"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['SUM'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat']);
							}
						}
					}
					if ($this->fieldsArr[$j]['name'] == $this->repGroupFields[$i]['strGroupField'])
					{
						...
						{
							$begin[GoodFieldName($gname.'_grval')] = htmlspecialchars($gvalue);
							if ($this->showGroupSummaryCount)
							{
								$end[GoodFieldName($gname.'_grval')] = htmlspecialchars($gvalue);
							}
						}
						else
						{
