...
function edit_error_handler($errno, $errstr, $errfile, $errline)
{
	...
	if ( $inlineedit ) 
		$message=""."record TIDAK diedit".". ".$errstr;
	else  
		$message="<<< "."record TIDAK diedit"." >>><br><br>".$errstr;
	$readevalues=true;
	$error_happened=true;
}
...
function add_error_handler($errno, $errstr, $errfile, $errline)
{
	...
	if ( $inlineadd!=ADD_SIMPLE ) 
		$message=""."record TIDAK ditambahkan".". ".$errstr;
	else  
		$message="<<< "."record TIDAK ditambahkan"." >>><br><br>".$errstr;
	$readavalues=true;
	$error_happened=true;
}
...
function GetDefaultValue($field,$table="")
{
	...
	if(!$table)
		$table=$strTableName;
	if($table=="agenda" && $field=="Tgl_mulai") 
	{
		return now();
	}
	if($table=="agenda" && $field=="Tgl_akhir") 
	{
		return now();
	}
	if($table=="agenda_hadir" && $field=="Jam") 
	{
		return now();
	}
	return "";
...
function CreateCKeditor($cfield, $value, $nWidth, $nHeight)
{
...
function WRGetAbsoluteFileName($path) 
{
	// get path to the root
	$pathToRoot = substr(dirname(__FILE__),0,strlen(dirname(__FILE__))-strlen("/include"));
	// cheks if there already we have absolute path
