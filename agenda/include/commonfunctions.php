...
function DisplayMap($params) 
{
	...
	$pageObject->googleMapCfg['mapsData'][$params['id']]['latField'] = $params['latField'] ? $params['latField'] : 0;
	$pageObject->googleMapCfg['mapsData'][$params['id']]['lngField'] = $params['lngField'] ? $params['lngField'] : 0;
	...
	$pageObject->googleMapCfg['mapsData'][$params['id']]['zoom'] = $params['zoom'] ? $params['zoom'] : 13;
...
function checkTableName($shortTName, $type=false)
{
	...
	if ("agenda_bos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("agenda_hadir" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("agenda_hasil" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("masuk" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("agenda_petugas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	...
	if ("Agenda_Pejabat" == $shortTName && ($type===false || ($type!==false && $type == 1)))
	{
		return true;	
	}
	return false;
...
function GetTablesList($pdfMode=false)
{
	...
	$strPerm = GetUserPermissions("agenda_bos");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="agenda_bos";
	}
	$strPerm = GetUserPermissions("agenda_hadir");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="agenda_hadir";
	}
	$strPerm = GetUserPermissions("agenda_hasil");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="agenda_hasil";
	}
	$strPerm = GetUserPermissions("masuk");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="masuk";
	}
	$strPerm = GetUserPermissions("agenda_petugas");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="agenda_petugas";
	}
	...
	$strPerm = GetUserPermissions("Agenda Pejabat");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Agenda Pejabat";
	}
...
function GetTableURL($table="")
{
	...
	if("agenda_bos"==$table) 
		return "agenda_bos";
	if("agenda_hadir"==$table) 
		return "agenda_hadir";
	if("agenda_hasil"==$table) 
		return "agenda_hasil";
	if("masuk"==$table) 
		return "masuk";
	if("agenda_petugas"==$table) 
		return "agenda_petugas";
	...
	if("admin_users"==$table) 
		return "admin_users";
	if("Agenda Pejabat"==$table) 
		return "Agenda_Pejabat";
}
...
{	
	...
	if("agenda"==$shortTName) 
		return "agenda";
	if("agenda_bos"==$shortTName) 
		return "agenda_bos";
	if("agenda_hadir"==$shortTName) 
		return "agenda_hadir";
	if("agenda_hasil"==$shortTName) 
		return "agenda_hasil";
	if("masuk"==$shortTName) 
		return "masuk";
	if("agenda_petugas"==$shortTName) 
		return "agenda_petugas";
	...
	if("admin_users"==$shortTName) 
		return "admin_users";
	if("Agenda_Pejabat"==$shortTName) 
		return "Agenda Pejabat";
}
...
// long binary data?
	...
	{
		$ret="Data Biner Panjang - tidak dapat ditampilkan";
	} 
	else
		$ret = $value;
	if($ret===false)
	...
	else if($format==FORMAT_FILE)
	{
		...
		if($strTableName=="agenda")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["id_agenda"]);
		}
		if($strTableName=="agenda_bos")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["id_bos"]);
		}
		if($strTableName=="agenda_hadir")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["id_hadir"]);
		}
		if($strTableName=="agenda_hasil")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["id_hasil"]);
		}
		if($strTableName=="masuk")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["no_urut"]);
		}
		if($strTableName=="agenda_petugas")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["id_user"]);
		}
		if($strTableName=="admin_rights")
		{
		}
		...
		if($strTableName=="Agenda Pejabat")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["id_bos"]);
		}
...
//		$ret.="pwin.document.write('" . db_addslashes(str_replace("\r\n","<br>",htmlspecialchars(substr($strValue,0, 801)))) ."');";
		$ret.="pwin.document.write('<br><hr size=1 noshade><a href=# onClick=\\'window.close();return false;\\'>"."Tutup jendela"."</a>');";
		$ret.="return false;\">"."Lanjut"." ...</a>";
	}
	else*/ if($cNumberOfChars>0 && strlen($strValue)>$cNumberOfChars && $mode==MODE_LIST)
	{
		...
		$ret = substr($strValue,0,$cNumberOfChars );
		$ret=htmlspecialchars($ret);
		$ret.=" <a href='javascript:void(0);' onClick=\"return DisplayPage(event,'fulltext.php','','','table=".GetTableURL($strTableName)."&".$iquery."','','');\">"."Lanjut"." ...</a>";
	}
...
function StrWhereAdv($strField, $SearchFor, $strSearchOption, $SearchFor2, $etype, $isSuggest=false)
{
	...
	$value2=false;
	if($strSearchOption=="Between")
		$value2=prepare_for_db($strField,$SearchFor2,$etype);
...
function ReadUserPermissions($userID="")
{
	...
	if($userID!="Guest")
	{
		$sql = "select GroupID from `agenda_ugmembers` where UserName='".db_addslashes($userID)."'";
	...
	$rights=array();
	$sql="select TableName,AccessMask from `agenda_ugrights` where GroupID in (".$groupstr.")";
	$rs = db_query($sql,$conn);
	while($data=db_fetch_numarray($rs))
	{
...
{	
	...
	if(array_key_exists("agenda",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("agenda_bos",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("agenda_hadir",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("agenda_hasil",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("masuk",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("agenda_petugas",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("admin_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("Agenda Pejabat",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
{
	global $cAdvSecurityMethod,$strTableName,$strOriginalTableName;
	...
	if($strOriginalTableName != GetFieldData($table, GetTableOwnerID($table), "ownerTable", ""))
		return "";
...
{	
	...
	switch($type)
	{
		...
		case EDIT_DATE_DD:
			...
			$jsControlObjectParams = $str.', yearVal: '.$time[0].', monthVal: '.$time[1].', dayVal: '.$time[2].'}';
...
//	dropdown control
		...
		elseif($LCType==LCT_CBLIST)
		{
			...
			foreach($arr as $opt)
			{
				echo '<input id="'.$cfield.'" type="checkbox" '.$alt.' name="'.$cfield.$postfix.'" value="'.htmlspecialchars($opt).'"';
				$res=array_search((string)$opt,$avalue);
				if(!($res===NULL || $res===FALSE))
...
//	add new item link
			if($addnewitem)
			{
				echo "&nbsp;<a href=# id='addnew_".$cfield."' onclick=\"".$celementvalue." return DisplayPage(event,'".$add_page."',".$id.",'".$cfield."','".jsreplace($field)."','".jsreplace($strTableName)."', parVal);\">"."Tambah"."</a>";
			}
			return;
		}
////////////////////////////////////////////////////////////////////////////////////////////
...
//	add new item
		if($addnewitem)
		{
			echo "&nbsp;<a href=# id='addnew_".$cfield."' onclick=\"return DisplayPage(event,'".$add_page."',".$id.",'".$cfield."','".jsreplace($field)."','".jsreplace($strTableName)."','');\">"."Tambah"."</a>";
		}
		return;
	}
////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////
	if($bUseCategory)
	{
		...
		if($addnewitem)
		{
			...
			echo "&nbsp;<a href=# id='addnew_".$cfield."' onclick=\"return DisplayPage(event,'".$add_page."',".$id.",'".htmlspecialchars(jsreplace($cfield))."','".jsreplace($field)."','".jsreplace($strTableName)."',".$celementvalue.");\">"."Tambah"."</a>";
		}
		return;
	}
////////////////////////////////////////////////////////////////////////////////////////////
...
//	add new item
	if($addnewitem)
		echo "&nbsp;<a href=# id='addnew_".$cfield."' onclick=\"return DisplayPage(event,'".$add_page."',".$id.",'".$cfield."','".htmlspecialchars(jsreplace($field))."','".htmlspecialchars(jsreplace($strTableName))."','');\">"."Tambah"."</a>";
...
function BuildEditControl($field , $value, $format, $edit, $fieldNum=0, $id="",$validate, $additionalCtrlParams, &$pageObj)
{
	...
	if($strTableName=="agenda")
	{
		$keylink.="&key1=".rawurlencode(@$keys["id_agenda"]);
		$iquery.=$keylink;
	}
	if($strTableName=="agenda_bos")
	{
		$keylink.="&key1=".rawurlencode(@$keys["id_bos"]);
		$iquery.=$keylink;
	}
	if($strTableName=="agenda_hadir")
	{
		$keylink.="&key1=".rawurlencode(@$keys["id_hadir"]);
		$iquery.=$keylink;
	}
	if($strTableName=="agenda_hasil")
	{
		$keylink.="&key1=".rawurlencode(@$keys["id_hasil"]);
		$iquery.=$keylink;
	}
	if($strTableName=="masuk")
	{
		$keylink.="&key1=".rawurlencode(@$keys["no_urut"]);
		$iquery.=$keylink;
	}
	if($strTableName=="agenda_petugas")
	{
		$keylink.="&key1=".rawurlencode(@$keys["id_user"]);
		$iquery.=$keylink;
	}
	if($strTableName=="admin_rights")
	{
	...
	else if($format==EDIT_FORMAT_TEXT_AREA)
	{
		...
		if(UseRTE($field))
		{
			$value = RTESafe($value);
												$str = substr($jsControlObjectParams, 0, -1);
			$jsControlObjectParams = $str.", useRTE: 'INNOVA'}";
			jscodeToAdd("RTEInnova",$jsControlObjectParams,$id, $pageObj);
			$browser="";
			if(@$_REQUEST["browser"]=="ie" && $nWidth<565)
				$nWidth=565;
			if(@$_REQUEST["browser"]=="ie")
				$browser="&browser=ie";
			echo "<iframe frameborder=\"0\" vspace=\"0\" hspace=\"0\" marginwidth=\"0\" marginheight=\"0\" scrolling=\"no\" id=\"".$cfield."\" ".(($edit==MODE_INLINE_EDIT || $edit==MODE_INLINE_ADD) && $is508==true ? "alt=\"".$strLabel."\" " : "")."name=\"".$cfield."\"title=\"Innova editor\"  style='width: " . ($nWidth+1) . "px;height: " . ($nHeight+1) . "px;'";
			echo " src=\"innova.php?table=".GetTableURL($strTableName)."&".($edit==MODE_INLINE_EDIT || $edit==MODE_INLINE_ADD ? "id=".$id."&" : '').$iquery.$browser."&".($edit==MODE_ADD || $edit==MODE_INLINE_ADD ? "action=add" : '')."\">";
			echo "</iframe>";
		}
		else{
...
//	filename edit
			if($format==EDIT_FORMAT_DATABASE_FILE && GetFilenameField($field))
			{
				...
				{
					$strfilename='<br><label for="filename_'.$cfieldname.'">'."Nama file".'</label>&nbsp;&nbsp;<input type="text" '.$inputStyle.' id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="20" maxlength="50" value="'.htmlspecialchars($filename).'">';					
				}
				else
				{
					$strfilename='<br><label for="filename_'.$cfieldname.'">'."Nama file".'</label>&nbsp;&nbsp;<input type="text" '.$inputStyle.' id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="20" maxlength="50" value="'.htmlspecialchars($filename).'">';					
			...
			$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="file0" checked>'."Simpan";
			if(strlen($value) && !IsRequired($field))
			{
				$strtype.='<input id="'.$ctype.'_delete" type="Radio" name="'.$ctype.'" value="file1">'."Hapus";
			}
...
//	if Add mode
			...
			if($format==EDIT_FORMAT_DATABASE_FILE && GetFilenameField($field))
			{
				$strfilename='<br><label for="filename_'.$cfieldname.'">'."Nama file".'</label>&nbsp;&nbsp;<input type="text" '.$inputStyle.' id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="20" maxlength="50">';			
			}
		}
...
//	filename edit
			...
			if(UseTimestamp($field))
				$filename_size=50;
			$strfilename='<input type=hidden name="filenameHidden_'.$cfieldname.'" value="'.htmlspecialchars($filename).'"><br>'."Nama file".'&nbsp;&nbsp;<input type="text" style="background-color:gainsboro" disabled id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="'.$filename_size.'" maxlength="100" value="'.htmlspecialchars($filename).'">';
			if ( $edit==MODE_INLINE_EDIT ) {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked onclick="/*$(\'[@id='.$cfield.']\').css(\'backgroundColor\',\'gainsboro\');$(\'[@id='.$cfield.']\')[0].disabled=true;*/">'."Simpan";
			} else {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked onclick="/*controlfilename'.$cfieldname.'(false)*/">'."Simpan";
			}


			if(strlen($value) && !IsRequired($field))
			{
				$strtype.='<input id="'.$ctype.'_delete" type="Radio" name="'.$ctype.'" value="upload1">'."Hapus";
			}
...
//	if Adding record		
			...
			$strfilename='<br>'."Nama file".'&nbsp;&nbsp;<input type="text" id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="'.$filename_size.'" maxlength="100">';			
		}
