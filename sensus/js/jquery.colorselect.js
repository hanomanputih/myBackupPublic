...
(function($) {
    ...
    function getDulyOffset(target, w, h) {
        ...
        if ((newpos.top + h) >= bh && bw > newpos.top) {
            ...
            // hardcode top of color select dropdown
            if (newpos.top < 0)
            	newpos.top = pos.top + height;
        }
        return newpos;
    }
    ...
    $.fn.colorselect = function(option) {        
        ...
        // added by Herman W
        // Reset html when inisiate more than one times
        me.html("");
        ...
        var aitem = '<a href="#" id="{1}-{0}" key="{2}" hidefocus="on"><em><span style="background:#{0};border:solid 1px #{3}" unselectable="on">&emsp;</span></em></a>';
        ...
        var blanka = '<a href="#" id="{1}-{0}" key="{2}" hidefocus="on"><em><span style=" text-align:center; width:46px;padding-top:2px; height:11px;border:solid 1px #8B7B8B;" unselectable="on">default</span></em></a>'
        var blank = [];
        blank.push("ffffff");
        blank.push(id);
        blank.push(-1);
