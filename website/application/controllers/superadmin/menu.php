...
class Menu extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $dataToView["content"] = $this->load->view("content/sadmin/dashboard/dashboard_menu",$datatosend,TRUE);
    ...
    {   
        ...
        $datatoview["ta_active"] = $this->ta->getTahunAjaranByAtribut(array("ta_status"=>"active"));
        $datatoview["ta_inactive"] = $this->ta->getTahunAjaranByAtribut(array("ta_status"=>"inactive"));
        $datatoview["content"] = $this->load->view("content/sadmin/pengaturan/akun/detail_akun",$datatosend,true);
        if($action == "edit")
        {
            $datatosend["akun"] = $this->user->getuserById($this->session->userdata("id_user"));
            $datatoview["content"] = $this->load->view("content/sadmin/pengaturan/akun/edit_akun",$datatosend,true);
        }
    ...
    public function ta($menu="",$id="")
    {
        ...
        if($menu == "tambah")
        {
            $datatoview["content"] = $this->load->view("content/sadmin/pengaturan/tahun_ajaran/add_tahun_ajaran",$datatosend,true);
        }
        else if($menu == "edit")
        {
        ...
        else if($menu == "hapus")
        {
            if(!empty($id))
            {
                ...
                redirect(base_url()."superadmin/menu/ta");
                exit;
            }
        }
        $this->load->view("template/template_sadmin",$datatoview);
    ...
    public function aktifasita()
    {
        ...
        $updateAll = $this->ta->updateAllTahunAjaran(array('ta_status'=>'inactive'));
        
        $update = $this->ta->updateTahunAjaran(array('ta_id'=>$id,'ta_status'=>'active'));
        
        if(($updateAll > 0) and ($update > 0))
        {
