...
class Pbo extends CI_Controller{
	...
	{
        redirect(base_url()."superadmin/pbo/praktikan","refresh");
        exit;
	}
	...
	public function praktikan($menu="",$id="")
	{
        ...
        $dataToView["content"] = $this->load->view("content/sadmin/pbo/praktikan/list_praktikan",$dataToSend,TRUE);

        if($menu == "tambah")
        {
            if($this->session->userdata("ta_status") == "active")
            {
            $dataToView["content"] = $this->load->view("content/sadmin/pbo/praktikan/add_praktikan","",TRUE);
            }
            else
            {
                redirect(base_url()."superadmin/pbo/praktikan");
                exit;
            }
        }
        else if($menu == "edit")
        {
            if(!empty($id))
            {
                ...
                $dataToView["content"] = $this->load->view("content/sadmin/pbo/praktikan/edit_praktikan",$dataToSend,TRUE);
            }
            else
            {
                redirect(base_url()."superadmin/pbo/praktikan","refresh");
	...
	public function kelas($menu="",$menu2="")
	{
        ...
        $dataToSend["kelas"] = $this->kelas->getKelasPboByAttr(array("ta_id"=>  $this->session->userdata("ta_id")));
        $dataToView["content"] = $this->load->view("content/sadmin/pbo/kelas/list_kelas",$dataToSend,TRUE);
        if($menu == "tambah")
        {
            if($this->session->userdata("ta_status") == "active")
            {
            $dataToView["content"] = $this->load->view("content/sadmin/pbo/kelas/add_kelas","",TRUE);
            }
            else
            {
                redirect(base_url()."superadmin/pbo/kelas");
                exit;
            }
        }
        else if($menu == "edit")
        {
            if($this->session->userdata("ta_status") == "active")
            {
                if(!empty($menu2))
                {
                    ...
                    $dataToView["content"] = $this->load->view("content/sadmin/pbo/kelas/edit_kelas",$dataToSend,TRUE);
                }
            }
            else
            {
        ...
        else if($menu == "hapus")
        {
            if(!empty($menu2))
            {
                ...
                redirect(base_url()."superadmin/pbo/kelas","refresh");
                exit;
            }
        }
        $this->load->view("template/template_sadmin",$dataToView);
	...
	public function nilai($menu="")
	{
		...
		$dataToView["content"] = $this->load->view("content/sadmin/nilai/list_nilai",$dataToSend,TRUE);
		if($menu == "tambah")
		{
			$dataToView["content"] = $this->load->view("content/sadmin/nilai/add_nilai",$dataToSend,TRUE);
		}
