...
class Asisten extends CI_Controller{
    ...
    public function index()
    {
        ...
        $dataToView["content"] = $this->load->view("content/sadmin/asisten/list_asisten",$dataToSend,TRUE);
...
//    imamsrifkan : fungsi untuk menambahkan data asisten
    public function tambah($action="")
    {
        if($this->session->userdata("ta_status") == "active")
        {
            ...
            $dataToView["content"] = $this->load->view("content/sadmin/asisten/add_asisten",$dataToSend,TRUE);
            $this->load->view("template/template_sadmin",$dataToView);
        }
        else
        {
            redirect(base_url()."superadmin/asisten");
            exit;
        }
    }
...
//    imamsrifkan : fungsi untuk edit asisten
    ...
    //        imamsrifkan : menyimpan hasil perubahan data user
        ...
        //            imamsrifkan : jika pasword juga diubah
            ...
            $dataToView["content"] = $this->load->view("content/sadmin/asisten/edit_asisten",$dataToSend,TRUE);
            $this->load->view("template/template_sadmin",$dataToView);
        }
        else
        {
...
//    imamsrifkan : fungsi untuk menghapus data asisten
    public function hapus($id="")
    {
            ...
            if($delete > 0)
            {
                redirect(base_url()."superadmin/asisten","refresh");
            }
            else
            {
                redirect(base_url()."superadmin/asisten","refresh");
